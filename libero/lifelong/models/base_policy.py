...
class BasePolicy(nn.Module, metaclass=PolicyMeta):
    def __init__(self, cfg, shape_meta):
        ...
        rgb_key = cfg.data.obs.modality.rgb[0]

        if self.cfg.aug:

            input_shape = shape_meta["all_shapes"][rgb_key]

            # color jitter

            policy_cfg.color_aug.network_kwargs["input_shape"] = input_shape
            color_aug = eval(policy_cfg.color_aug.network)(
                **policy_cfg.color_aug.network_kwargs
            )

            # affine / translation
            policy_cfg.translation_aug.network_kwargs["input_shape"] = input_shape
            translation_aug = eval(policy_cfg.translation_aug.network)(
                **policy_cfg.translation_aug.network_kwargs
            )

            # random erase
            policy_cfg.erase_aug.network_kwargs["input_shape"] = input_shape
            erase_aug = eval(policy_cfg.erase_aug.network)(
                **policy_cfg.erase_aug.network_kwargs
            )

            self.img_aug = DataAugGroup((color_aug, translation_aug, erase_aug))
    ...
    def _get_img_tuple(self, data):
        img_tuple = tuple(
            [data["obs"][img_name] for img_name in self.rgbkey.keys()]
        )
        return img_tuple
    ...
    def _get_aug_output_dict(self, out):
        img_dict = {
            img_name: out[idx]
            for idx, img_name in enumerate(self.rgbkey.keys())
        }
        return img_dict
    ...
    def preprocess_input(self, data, train_mode=True):
        if self.cfg.train.use_augmentation:
            ...
            for img_name in self.rgbkey.keys():
    ...
    def compute_loss(self, data,taskid, idx, reduction="mean", save_feature=False,saving_dir=None):

        if save_feature is True:
            data = self.preprocess_input(data)
            dist = self.forward(data)

            task_dir = os.path.join(saving_dir, str(taskid))
            os.makedirs(task_dir, exist_ok=True)
            save_path = os.path.join(task_dir, f"{idx}.pt")

            torch.save({
                "actions": data["actions"].cpu(),
                "feature": dist.cpu()
            }, save_path)
        ######################################################################################################
        else:
            ...
            dist,last_feat = self.forward(data["feature"].squeeze(1))
            loss = self.policy_head.loss_fn(dist, data["actions"].squeeze(1), reduction)
            return loss,last_feat
