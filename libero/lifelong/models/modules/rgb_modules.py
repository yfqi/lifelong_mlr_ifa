...
class CLIPVisionEncoder(nn.Module):
    def __init__(self, vision_model_name="openai/clip-vit-base-patch16", output_size=64, input_shape=None,adaption=False, linear=False, **kwargs):
        super().__init__()
        self.clip = CLIPVisionModel.from_pretrained(vision_model_name)
        if not adaption:
        ...
        self.clip_transforms = transforms.Compose([
            transforms.Resize(224),                     # 或者 (224,224)
        ...
        if self.linear == True:
            ...
            hidden_size = clip.config.vision_config.hidden_size   # 768
            output_size = clip.config.projection_dim             # 512

            self.projection: nn.Linear = clip.visual_projection

            for p in self.projection.parameters():
                p.requires_grad = False  # adaption=True 则微调，否则冻结

    def forward(self, image, langs=None):
        """
        image: (B, C, H, W) in [0,1] range (normalized elsewhere if needed)
        return: (B, output_size)
        """
        # vision_outputs = self.clip.vision_model(pixel_values=image) # the same as the following sentence
        pixel_inputs = self.clip_transforms(image)
        vision_outputs = self.clip(pixel_values=pixel_inputs)
        pooled = vision_outputs.pooler_output  # (B, hidden_size)
        if self.linear:
            pooled = self.projection(pooled)     # (B, 512)
        return pooled       # (B, output_size)